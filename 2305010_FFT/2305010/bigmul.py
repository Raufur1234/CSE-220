...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    for end in range(len(digits),0,-base_digits):
       ...
       limbs_arr.append(int(digits[start:end]))
        
    limbs =np.array(limbs_arr,dtype=np.int64)
    return sign,limbs
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    top = len(final_limbs) - 1
    while top > 0 and final_limbs[top] == 0:
        top -= 1
    final_limbs = final_limbs[:top + 1]

    if len(final_limbs) == 1 and final_limbs[0] == 0:
        return "0"

    number = str(final_limbs[-1])
    for limb in final_limbs[-2::-1]:
        number += str(limb).zfill(base_digits)
...
def multiply_transform(a, b, engine):
    ...
    N = linear_len if engine.name == "arbitrary" else next_power_of_two(linear_len)
    padded_a= np.pad(a, (0, N-len(a)), mode='constant', constant_values=0)
    padded_b= np.pad(b, (0, N-len(b)), mode='constant', constant_values=0)

    spectrum_a=engine.transform(padded_a)
    spectrum_b=engine.transform(padded_b)

    product_spectrum=spectrum_a*spectrum_b
    product=engine.inverse(product_spectrum)

    rounded_real=np.round(product.real)

    return rounded_real.astype(np.int64), N
...
def run_single(path, method, out_dir):
    ...
    def digit_count(text):
        return len(text) - 1 if text[0] in "+-" else len(text)

    expected = int(text_a) * int(text_b)
    verdict = "MATCH" if int(product) == expected else "MISMATCH"
...
FFT_SIZES = [128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144]
